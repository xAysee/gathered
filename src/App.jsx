...
async function sbVerifyCode(email, code) {
  ...
  // Server returns a magic link -- navigate to it to establish a real Supabase session
  if (data.link) {
    window.location.href = data.link;
    return null; // page will reload via the magic link redirect
  }
  return null;
}
...
async function sbInsertRecipe(userId, r) {
  const { data, error } = await supabase.from("recipes").insert({
    ...
    tags: r.tags||[], notes: r.notes||"", image: r.image||null,
    ingredients: r.ingredients||[], steps: r.steps||[], folder: r.folder||null,
  }).select().single();
...
async function sbMoveToFolder(recipeId, folder) {
  const { error } = await supabase.from("recipes").update({ folder: folder||null }).eq("id", recipeId);
  if (error) console.error("sbMoveToFolder error:", error);
}
...
function AuthScreen({ T, onLogin }) {
  ...
  async function verifyCode() {
    ...
    const key = email.trim().toLowerCase();
    try {
      // Verification happens server-side -- returns null and redirects via magic link
      await sbVerifyCode(key, codeInput);
      // If no redirect happened (shouldn't occur), show a message
              ...
              : <><button className="bg" onClick={()=>setShowShare(true)} style={{fontSize:13}}>↗ Share</button>
                  <div style={{position:"relative",display:"inline-block"}}>
                    <button className="bg" onClick={()=>setShowFolderMenu(f=>!f)} style={{fontSize:13}}>
                      📁 {recipe.folder||"Move to folder"}
                    </button>
                    {showFolderMenu&&<div style={{position:"absolute",top:"110%",right:0,zIndex:50,background:T.surface,border:`1px solid ${T.border}`,borderRadius:8,boxShadow:"0 4px 16px rgba(0,0,0,.15)",minWidth:180,padding:"4px 0"}}>
                      <button onClick={()=>{onMoveToFolder&&onMoveToFolder(recipe.id,null);setShowFolderMenu(false);}}
                        style={{width:"100%",textAlign:"left",padding:"7px 14px",background:"none",border:"none",cursor:"pointer",fontFamily:sans,fontSize:13,color:recipe.folder?T.ink:T.terra,fontWeight:recipe.folder?"400":"600"}}>
                        📚 All Recipes {!recipe.folder&&"✓"}
                      </button>
                      {folders.map(f=>(
                        <button key={f} onClick={()=>{onMoveToFolder&&onMoveToFolder(recipe.id,f);setShowFolderMenu(false);}}
                          style={{width:"100%",textAlign:"left",padding:"7px 14px",background:"none",border:"none",cursor:"pointer",fontFamily:sans,fontSize:13,color:recipe.folder===f?T.terra:T.ink,fontWeight:recipe.folder===f?"600":"400"}}>
                          📁 {f} {recipe.folder===f&&"✓"}
                        </button>
                      ))}
...
// FOLDER PANEL
// ═══════════════════════════════════════════════════════════════════════════════
function FolderPanel({ folders, activeFolder, recipes, T, sans, serif, onSelect, onCreate, onDelete }) {
  ...
  function countInFolder(name) {
    return recipes.filter(r => r.folder === name).length;
  }
        ...
        {folders.map(f=>(
          <div key={f} style={{display:"flex",alignItems:"center",group:true}}>
            <button onClick={()=>onSelect(f)}
              style={{flex:1,textAlign:"left",padding:"7px 16px",background:activeFolder===f?T.terraLight:"transparent",color:activeFolder===f?T.terra:T.ink,border:"none",cursor:"pointer",fontFamily:sans,fontSize:13,fontWeight:activeFolder===f?600:400,display:"flex",justifyContent:"space-between",alignItems:"center",minWidth:0}}>
...
export default function App() {
  ...
  async function deleteFolder(name) {
    ...
    setSettings(newSettings);
    if (user) await sbSaveSettings(user.id, newSettings);
    // Move recipes in this folder back to All Recipes
    const affected = recipes.filter(r => r.folder === name);
    for (const r of affected) {
      await sbMoveToFolder(r.id, null);
      setRecipes(prev => prev.map(x => x.id === r.id ? {...x, folder: null} : x));
    }
  }

  async function moveRecipeToFolder(recipeId, folder) {
    await sbMoveToFolder(recipeId, folder);
    setRecipes(prev => prev.map(r => r.id === recipeId ? {...r, folder: folder||null} : r));
  }
  ...
  const folderFiltered = activeFolder === null ? recipes : recipes.filter(r => r.folder === activeFolder);
  ...
  return (
    <>
      ...
      <div style={{minHeight:"100vh",background:T.bg}}>
        ...
        {page==="recipes"&&(
          <div style={{maxWidth:1100,margin:"0 auto",padding:isMobile?"14px 10px":"20px 18px",display:"flex",gap:16,alignItems:"flex-start"}}>
            ...
            {!isMobile&&(
              <aside style={{width:200,flexShrink:0,background:T.surface,border:`1px solid ${T.border}`,borderRadius:10,padding:"12px 0",position:"sticky",top:68}}>
                <FolderPanel
      ...
      {selected&&<RecipeDetail recipe={selected} settings={settings} T={T} currentUid={uid} currentName={userName} folders={folders} onMoveToFolder={moveRecipeToFolder} onClose={()=>setSelected(null)} onDelete={id=>{deleteRecipe(id);setSelected(null);}} onSave={saveRecipe}/>}
